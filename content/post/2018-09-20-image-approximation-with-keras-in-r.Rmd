...
To honour the name of this blog, let's pick an image of the bird species godwit (limosa) and load it using `imager`'s `load.image`. 
...
Plotting the image is easy. We choose to show the image without axes for aestetic reasons.
...
load.image("trainplot.png") %>% plot(axes=F)
```
...
```
```{r echo=FALSE, message=FALSE, warning=FALSE}
load.image("model1.png") %>% plot(axes=F)
```
...
```
```{r echo=FALSE, message=FALSE, warning=FALSE}
load.image("model2.png") %>% plot(axes = F)
```
...
```
```{r echo=FALSE, message=FALSE, warning=FALSE}
load.image("model3.png") %>% plot(axes=F)
```
